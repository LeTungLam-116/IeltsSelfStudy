...
const QuestionDetailsPage = memo(function QuestionDetailsPage() {
  ...
  useEffect(() => {
    const fetchQuestion = async () => {
      ...
      try {
        ...
        showError('Error', 'Failed to load question details. Please try again.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const renderOptions = (optionsJson: string | null) => {
    ...
    try {
      ...
      return <div className="text-red-600">Invalid options format</div>;
    }
  };
  ...
  if (isLoading) {
    return <Loading text="Loading question details..." />;
  }
  ...
  return (
    <div className="space-y-8">
      <header className="flex items-center justify-between">
        <Button variant="ghost" onClick={() => navigate(-1)} className="flex items-center gap-2">
          <IconArrowLeft className="w-5 h-5" /> Back to Questions
        </Button>
        <h1 className="text-3xl font-bold text-gray-900">Question #{question.questionNumber} Details</h1>
        <Button onClick={() => setShowEditModal(true)} className="flex items-center gap-2">
          <IconEdit className="w-5 h-5" /> Edit Question
        </Button>
      </header>
          ...
          <Card className="p-6">
            ...
            <div className="space-y-3">
              <div>
                ...
                <div className="mt-1">
                  <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${
                    question.questionType === 'MultipleChoice' ? 'bg-blue-100 text-blue-800' :
              ...
              <div>
                ...
                <div className="mt-1">
                  <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${
                    question.skill === 'Listening' ? 'bg-blue-100 text-blue-800' :
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-gray-600">ID: {question.exerciseId}</p>
              </div>
            </div>
          </Card>
